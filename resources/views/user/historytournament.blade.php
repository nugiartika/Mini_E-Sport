...
<div class="row pt-4">
    ...
    $findIsSuccess = $team->teamTournament->transaction()->whereIn('status', ['PAID', 'UNPAID', 'PENDING']);
    ...
    <div class="col-sm-6 col-xxl-4 mb-3">
        <div class="card">
            <img src="{{ asset("storage/{$team->tournament->images}") }}" alt="{{ $team->tournament->name }}" class="card-img-top" />
            <div class="card-body">
                <div class="d-flex gap-3 mb-3 justify-content-between align-items-center">
                    ...
                    <a href="{{ route('transaction.create', ['tournament_id' => $team->id]) }}" class="btn btn-sm btn-primary">Bayar Sekarang</a>
                    @endif
                </div>
                ...
                <div class="d-flex gap-3 border-top justify-content-between pt-3">
                    ...
                    <span class="badge bg-{{ \App\Enums\TransactionStatus::color($latestTransaction->status) }}">{{ \App\Enums\TransactionStatus::label($latestTransaction->status) }}</span>
                    @else
    ...
    <div class="col-lg-12">
        <center>
            <img src="{{ asset('assets/img/No-data.png') }}" alt="" style="display: block; margin: 0 auto; max-width: 20%; height: auto;">
        </center>
